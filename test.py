...
def evaluation(encoder, bn, decoder, dataloader, device, _class_=None, dataset=None):
    ...
    if dataset == "CIFAR-10":
        with torch.no_grad():
            for img, label in dataloader:
                ...
                gt_list_sp.append(np.array(label))
...
def visualization(_class_, dataset=None):
    print(_class_)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(device)
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> None:
    ...
    for th in np.arange(min_th, max_th, delta):
        ...
        df1 = pd.DataFrame([[mean(pros), fpr, th]], columns=["pro", "fpr", "threshold"])
